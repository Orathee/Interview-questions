...
- 初始容量DEFAULT_INITIAL_CAPACITY=1<<4=16,扩容newsize = oldsize*2，size一定为2的n次幂
...
- 底层数组+链表实现，无论key还是value都**不能为null**，线程**安全**，实现线程安全的方式是在修改数据时锁住整个HashTable，效率低，ConcurrentHashMap做了相关优化
  ...
  ```
   ...
   ...
   public synchronized V put/remove/clear/repalce/...(K key, V value)
   ...
